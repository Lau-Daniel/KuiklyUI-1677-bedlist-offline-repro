...
import com.tencent.kuikly.compose.ui.geometry.isSimple
...
sealed class Outline {
    ...
    class Rounded(val roundRect: RoundRect) : Outline() {
         ...
         */
        internal val roundRectPath: Path by lazy(LazyThreadSafetyMode.NONE) {
...
private inline fun DrawScope.drawOutlineHelper(
    ...
    is Outline.Rounded -> {
        ...
        if (!outline.roundRect.isSimple) {
            drawPathBlock(outline.roundRectPath)
        } else {
...
fun Canvas.drawOutline(outline: Outline, paint: Paint) = when (outline) {
    ...
    is Outline.Rounded -> {
        ...
        // Canvas primitive for drawing a rounded rectangle
        if (!outline.roundRect.isSimple) {
            drawPath(outline.roundRectPath, paint)
        } else {
            drawRoundRect(
